...
export async function transcribeAudio(audioAsBase64: string, mimeType: string){
    const response = await gemini.models.generateContent({
        ...
        contents: [
            {
                text: 'Trasncreva o áudio para português do Brasil. Seja preciso e natural na transcirção. Mantenha a pontuação adequada e divida o texto em parágrafos quando for apropriado.'
            },
            {
                inlineData:{
                    mimeType,
...
export async function generateEmbeddings(text: string){
    ...
    if(!response.embeddings?.[0].values){
        throw new Error("Não foi possivel gerrar os embeddings.")
    }
...
export async function generateAnswer(question: string, transcriptions: string[]){
    ...
    const prompt = `
        ...
        PERGUNTA: ${question}
        INSTRUÇÕES:
        - Use apenas informações contidas no contexto eviado;
        ...
        - Se for citar o contexto, utilizer o termo "conteúdo da aula"
